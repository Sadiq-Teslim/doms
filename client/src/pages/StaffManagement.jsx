...
const SORT_OPTIONS = [
  ...
  { label: "Department", value: "department_asc" },
  { label: "Terminal", value: "terminal_asc" },
];
...
export default function StaffManagement() {
  ...
  const [data, setData] = useState({ rows: [], total: 0, page: 1, pageSize: 12, departments: [] });
  ...
  useEffect(() => {
    const t = setTimeout(() => {
      ...
      api.get(`/staff?${params.toString()}`).then((r) => setData(r.data)).catch(() => {}).finally(() => setLoading(false));
    }, q ? 300 : 0);
    return () => clearTimeout(t);
  }, [q, sort, department, terminal, page]);

  useEffect(() => { setPage(1); }, [q, sort, department, terminal]);
  ...
  return (
    <div className="mx-auto max-w-[1100px] space-y-6">
      <h1 className="text-[20px] font-semibold text-[#2D2922]">Staff Management</h1>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex min-w-[260px] max-w-[420px] flex-1 items-center gap-2 rounded-[12px] border border-[#D1D1D1] px-3 py-2.5 text-[#6B665E]">
          <Search />
          <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search staff" className="w-full bg-transparent text-[14px] text-[#2D2922] outline-none placeholder:text-[#B7B2AA]" />
        </div>
        <div className="flex items-center gap-3">
          ...
          <div className="relative">
            <button onClick={() => setFilterOpen((v) => !v)} className="relative flex items-center gap-2 rounded-[12px] border border-[#D1D1D1] px-4 py-2.5 text-[14px] text-[#6B665E]">
              Filter <FilterIcon />
              {(department || terminal) && <span className="absolute -right-1 -top-1 h-2.5 w-2.5 rounded-full bg-[#FDB706]" />}
            </button>
            {filterOpen && (
              <>
                ...
                <div className="absolute right-0 top-[calc(100%+6px)] z-20 w-64 rounded-[12px] bg-white p-4 shadow-[0_4px_20px_rgba(0,0,0,0.15)]">
                  <p className="mb-2 text-[12px] font-medium uppercase tracking-wide text-[#A39E96]">Department</p>
                  <div className="mb-4 flex flex-wrap gap-2">
                    <button onClick={() => setDepartment("")} className={`rounded-full px-3 py-1 text-[13px] ${!department ? "bg-[#FDB706] text-black" : "border border-[#D1D1D1] text-[#6B665E]"}`}>All</button>
                    {(data.departments || []).map((d) => (
                      <button key={d} onClick={() => setDepartment(d)} className={`rounded-full px-3 py-1 text-[13px] ${department === d ? "bg-[#FDB706] text-black" : "border border-[#D1D1D1] text-[#6B665E]"}`}>{d}</button>
                    ))}
                  </div>
                  <p className="mb-2 text-[12px] font-medium uppercase tracking-wide text-[#A39E96]">Terminal</p>
                  <div className="flex flex-wrap gap-2">
                    <button onClick={() => setTerminal("")} className={`rounded-full px-3 py-1 text-[13px] ${!terminal ? "bg-[#FDB706] text-black" : "border border-[#D1D1D1] text-[#6B665E]"}`}>All</button>
                    {TERMINALS.map((tm) => (
                      <button key={tm} onClick={() => setTerminal(tm)} className={`rounded-full px-3 py-1 text-[13px] ${terminal === tm ? "bg-[#FDB706] text-black" : "border border-[#D1D1D1] text-[#6B665E]"}`}>{tm}</button>
                    ))}
                  </div>
                  {(department || terminal) && (
                    <button onClick={() => { setDepartment(""); setTerminal(""); }} className="mt-4 text-[13px] font-medium text-[#A57506] hover:underline">Clear filters</button>
                  )}
                </div>
              </>
            )}
        ...
        <table className="w-full min-w-[760px] border-collapse">
          <thead>
            <tr className="bg-[#F1F1F1] text-left text-[14px] font-medium text-[#6B665E]">
              <th className="rounded-l-md px-4 py-2.5">Staff Name</th>
              <th className="px-4 py-2.5">Department</th>
              <th className="px-4 py-2.5">Role</th>
              <th className="rounded-r-md px-4 py-2.5">Terminal</th>
            </tr>
          </thead>
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={4} className="py-10 text-center text-[14px] text-[#A39E96]">No staff found.</td></tr>
            ) : (
              rows.map((r) => (
                <tr key={r.id} className="border-b border-[#F1F1F1] text-[16px] text-[#2D2922]">
                  ...
                  <td className="px-4 py-3">{r.terminal}</td>
                </tr>
              ))
            )}
          </tbody>
